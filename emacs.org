...
ctrl + x c  -> cleanup buffer list (kill unused)
...
ctrl + x ctrl + k r -> run last macro on each line of region
...
* Dvorak                                                              :emacs:
alt + 1 - turn dvorak mode off for one buffer
alt + 2 - turn dvorak mode on for one buffer
* Misc 								      :emacs:
ctrl + g                      -> Cancel
ctrl + x u                    -> UNDO tree
